		...
		static DebugLogConsole()
        {
            ...
            var userAssemblies = AppDomain.CurrentDomain
                ...
                .Where(a => !a.IsDynamic 
                            ...
                            && a.GetCustomAttribute<AssemblyCompanyAttribute>()?.Company.ToLower() != "unity technologies"
                            && a.GetCustomAttribute<AssemblyCompanyAttribute>()?.Company.ToLower() != "mono development team"
                            && !a.FullName.ToLower().StartsWith("unity")
                            && !a.FullName.ToLower().StartsWith("icsharpcode")
                            && !a.FullName.ToLower().StartsWith("excss")
                            && !a.FullName.ToLower().StartsWith("nunit")
                )
                .ToList();
            var methodAttr = userAssemblies
                .SelectMany(a => a.GetExportedTypes())
                .SelectMany(t => t.GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.DeclaredOnly))
                .Select(m => (method: m,
                    attr: m.GetCustomAttributes(typeof(ConsoleMethodAttribute), false).OfType<ConsoleMethodAttribute>()
                        .FirstOrDefault()))
                .Where(m => m.attr != null)
                .ToList();
            stopWatch.Stop();
            Debug.Log($"Time to iterate ConsoleMethod attributes: {stopWatch.ElapsedMilliseconds} ms");
