...
public class GoodingSolver {
    ...
    /*
     * GM = Gravitational constant * Mass of the primary body
     * R1 = position at t0
     * V1 = velocity at t0
     * R2 = position at t1
     * V2 = velocity at t1
     ...
     */
    public static (Vector3 Vi, Vector3 Vf) Solve(float GM, Vector3 R1, Vector3 V1, Vector3 R2, float tof, int nrev) {
        ...
        double theta = Math.Acos(Clamp(Vector3.Dot(ux1, ux2), -1.0, 1.0));

        /* calculate the angle between the orbit normal of the initial orbit and the fundamental reference plane */

        double angle_to_on = Math.Acos(Clamp(Vector3.Dot(ur1xv1, uz1), -1.0, 1.0));
        ...
        if (nrev > 0 && n > 1) {
            Vi = (float)VR21 * ux1 + (float)VT21 * uy1;
            Vf = (float)VR22 * ux2 + (float)VT22 * uy2;
        } else {
            Vi = (float)VR11 * ux1 + (float)VT11 * uy1;
            Vf = (float)VR12 * ux2 + (float)VT12 * uy2;
        }
